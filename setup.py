...
NB_EXT_DIR = os.path.join(os.path.dirname(__file__), 'stepsize/stepsize_nb-ext')
CUR_NB_EXT = os.path.join(NB_DIR, 'stepsize_nb-ext')

SERVER_DIR = os.path.join(os.path.expanduser('~'), '.ipython/extensions')
SERVER_EXT_DIR = os.path.join(os.path.dirname(__file__), 'stepsize/stepsize_server-ext')
CUR_SERVER_EXT = os.path.join(SERVER_DIR, 'stepsize_server-ext')
SERVER_EXT_CONFIG = [
	"c.NotebookApp.allow_credentials = True",
	"c.NotebookApp.allow_origin_pat = '(http\://localhost:[0-9]{4}.*|http\://127\.0\.0\.1:[0-9]{4}.*)'",
	"import os",
	"import sys",
...
class InstallCommand(install):
	...
	def setup_extensions(self,CUR_EXT, EXT_DIR, DIR):
		...
		if os.path.exists(CUR_EXT):
			question = "ACTION: The Stepsize extension directory already exists, do you want to " \
					"overwrite %s with the current installation?" % (CUR_EXT)
			prompt = self.query(question)
			if prompt == "yes":
				try:
					install_nbextension(EXT_DIR, overwrite=True, nbextensions_dir=DIR)
					print "OUTCOME: Added the extension to your " \
					 	  "%s directory" % (DIR)
				except:
					print "WARNING: Unable to install the extension to your (nb)extensions folder"
	...
	def setup_notebook_config(self):
		...
		try:
			...
			print "OUTCOME: Added the Stepsize notebook extension configuration to the " \
				  "notebook.json"
		except:
			print "WARNING: An error occured when trying to add " \
				  "%s to the notebook.json" % (data)
			print "ERROR: %s" % (sys.exc_info()[0])
			raise


	def setup_server_config(self, EXT_CONFIG):
		# Set the server extension to launch on startup by modifying the jupyter_notebook_config.py
		...
		with open(CONFIG, 'r+') as fh:
			...
			for i in EXT_CONFIG:
				if i not in lines:
					...
					fh.write('\n')
					fh.write(i)
		print "OUTCOME: Added the Stepsize server extension configuration to the " \
			  "jupyter_config.py"
	...
	def setup_uid(self, UID_DIR, CUR_UID, UID):
		if os.path.isfile(CUR_UID):
			...
			print "OUTCOME: Added the Stepsize extension installation identifier to " \
				  "%s" % (os.path.join(os.path.expanduser('~'), '.ipython/extensions'))
	...
	def write_uid(self, CUR_UID):
		...
		if entry['uid'] == "xxxx":
			...
			with open(CUR_UID, 'w') as outfile:
				...
				print "OUTCOME: Set the Stepsize extension installation identifier"
