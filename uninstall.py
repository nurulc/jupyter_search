...
NB_CONFIG = os.path.join(ConfigManager()._config_dir_default(), 'notebook.json')
	...
	if os.path.exists(CUR_EXT):
		try:
			...
			print "WARNING: Unable to uninstall the extension in your " \
				  "%s directory" % (DIR)
			print "ERROR: %s, at line %d" % (sys.exc_info()[0], sys.exc_traceback.tb_lineno)
	else:
		print "WARNING: Unable to uninstall the Stepsize extension since it does not exist in " \
			  "%s" % (DIR)
	...
	if os.path.isfile(CONFIG):
		...
		try:
			if open(CONFIG).read() != "":
				...
				if count > 0:
					print "OUTCOME: Removed the Stepsize extension configuration from the notebook.json"
		except:
			print "WARNING: An error occured when trying to remove " \
				  "%s from the notebook.json" % (data_element)
			print "ERROR: %s, at line %d" % (sys.exc_info()[0], sys.exc_traceback.tb_lineno)
	else:
		print "WARNING: Unable to remove the Stepsize extension configuration since the " \
			  "notebook.json does not exist in %s" % (ConfigManager()._config_dir_default())
...
def remove_server_config(CONFIG, EXT_CONFIG):
	...
	if os.path.isfile(CONFIG):
		...
		try:
			...
			f.close()
			if count > 0:
				print "OUTCOME: Removed the Stepsize extension configuration from the " \
					  "jupyter_config.py"
		except:
			print "WARNING: An error occured when trying to remove " \
				  "the Stepsize server configuration from the jupyter_config.py"
			print "ERROR: %s, at line %d" % (sys.exc_info()[0], sys.exc_traceback.tb_lineno)
	else:
		print "WARNING: Unable to remove the Stepsize extension configuration since the " \
			  "jupyter_config.py does not exist in %s" % (jupyter_config_dir())


def remove_egg():
	# Removes the Stepsize.egg-info
	EGG_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'Stepsize.egg-info')
	if os.path.exists(EGG_DIR):
		...
		print "WARNING: Unable to remove the Stepsize.egg-info since it does not exist in the " \
			  "directory"
